...
framework.OpenSession(cache, config, ...)
    ...
    │
    └─ 按 config.Tiers 顺序遍历插件：
            for tier in config.Tiers:
                for pluginOption in tier.Plugins:
                    plugin = GetPluginBuilder(pluginOption.Name)(pluginOption.Arguments)
                    plugin.OnSessionOpen(ssn)   ← 插件注册回调
    │
    ▼
runOnce：执行 Actions（Allocate、Reclaim、Preempt 等）
    │
    │  各 Action 内部会调用：
    │  - ssn.QueueOrderFn、JobOrderFn、TaskOrderFn、PodSetOrderFn
    │  - ssn.SubsetNodesFn
    │  - ssn.PrePredicateFn、PredicateFn
    │  - ssn.OrderedNodesByTask → ssn.NodeOrderFn
    │  - ssn.FittingNode → ssn.PredicateFn
    │  - ssn.CanReclaimResources、ReclaimVictimFilter、PreemptVictimFilter
    │  - ssn.ReclaimScenarioValidatorFn、PreemptScenarioValidator
    │  - ssn.IsJobOverQueueCapacityFn、IsNonPreemptibleJobOverQueueQuotaFn
    │  - ssn.MutateBindRequestAnnotations
    │
    ▼
