...
### 5.2 App.Run：注册 BindRequestReconciler

```go
binder := binding.NewBinder(app.Client, app.rrs, app.plugins)
reconciler := controllers.NewBindRequestReconciler(
	app.manager.GetClient(), app.manager.GetScheme(), app.manager.GetEventRecorderFor("binder"),
	app.reconcilerParams, binder, app.rrs)
reconciler.SetupWithManager(app.manager)
```

- **Binder**：执行实际绑定逻辑。
- **BindRequestReconciler**：controller-runtime 的 Reconcile 循环，Watch BindRequest，触发 Binder.Bind。
...
- 若配置了 `BackoffLimit`：`RequeueAfter = (1 << FailedAttempts) * time.Second`，指数退避重试。
